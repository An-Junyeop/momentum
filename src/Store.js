...
class Store {
    constructor(localStorage) {
        ...
        if (
            ...
            this.localStorage.getItem(USERNAME) === null
        ) {
        ...
        if (
            ...
            this.localStorage.getItem(LIST) === null
        ) {
    ...
    /* To do 아이템 업데이트 */
    updateTodoList(id) {
        const todoList = JSON.parse(this.localStorage.getItem(LIST)).map(
    ...
    /* To do 아이템 삭제 */
    removeTodoItem(id) {
        const todoList = JSON.parse(this.localStorage.getItem(LIST)).filter(
            (item) => item.id !== id,
        );
